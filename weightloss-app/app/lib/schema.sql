...
DROP INDEX IF EXISTS xp_events_source;
CREATE UNIQUE INDEX IF NOT EXISTS xp_events_source ON xp_events(user_id, source_table, source_id) WHERE source_table IS NOT NULL;
...
-- Full gamification reset for all users.
-- Wipes xp_events, achievements, streaks, quests, challenges, journeys.
-- Preserves weigh-ins, food logs, water logs, exercise logs, photos, notes.
UPDATE user_levels SET total_xp = 0, level = 1, updated_at = now();
UPDATE user_stats  SET total_xp = 0, current_level = 1, longest_streak = 0,
  achievements_unlocked = 0, total_days_logged = 0, total_water_ml = 0,
  total_calories_tracked = 0, challenges_completed = 0, total_weight_kg_lost = 0,
  journeys_completed = 0, streak_protection_tokens = 0, updated_at = now();
UPDATE streaks SET current_count = 0, longest_count = 0, last_activity_date = NULL, updated_at = now();
DELETE FROM xp_events;
DELETE FROM user_achievements;
DELETE FROM daily_quests;
DELETE FROM weekly_challenges;
DELETE FROM journey_progress;
-- Also remove any corrupt weigh-ins (< 20 kg is impossible for adult humans)
